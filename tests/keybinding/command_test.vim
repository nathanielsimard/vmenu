...
function s:Test.test_WhenExecute_ShouldCloseMenu()
    let l:keybinding = vmenu#keybinding#command#new(s:KEY, s:DESCRIPTION, s:VALID_COMMAND, self.menu)
...
function s:Test.test_GivenValidCommand_WhenExecute_ShouldExecuteCommand()
    ...
    let l:keybinding = vmenu#keybinding#command#new(s:KEY, s:DESCRIPTION, l:command, self.menu)
